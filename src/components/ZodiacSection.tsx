...
export default function ZodiacSection() {
  ...
  // Auto-scroll every 3s unless paused
  useEffect(() => {
  ...
  const fetchHoroscope = async (sign: string, type: "daily" | "weekly" | "monthly") => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/api/v1/horoscope/generate", {
        ...
        body: JSON.stringify({ sign, type }),
      });

      const text = await response.text();
      if (!response.ok) throw new Error(text || "Failed to fetch horoscope");
  ...
  // Fetch horoscope whenever sign/tab changes
  useEffect(() => {
      ...
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800">
            {t("todayPrediction", "Today's Astrology Prediction")}
          </h2>
        </div>
          ...
          >
            {zodiacKeys.map((key) => (
              <div
                ...
                onClick={() => {
                  setSelectedZodiacSign(key);
                  setActiveHoroscopeTab("daily");
                }}
              >
                <img src={zodiacIcons[key]} alt={key} className="w-16 h-16 mx-auto object-contain" />
        ...
        >
          ...
          >
            ...
            <div className="flex justify-center mb-8 px-4 md:px-0">
              <div className="bg-white rounded-xl p-2 shadow-lg flex flex-wrap justify-center gap-2">
                {(["daily", "weekly", "monthly"] as const).map((period) => (
                  ...
                  >
                    {t(`tabs.${period}`, period.charAt(0).toUpperCase() + period.slice(1))}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mx-4 md:mx-0 min-h-[120px] flex items-center justify-center">
              {loading && <p className="text-center text-gray-600">{t("loading", "Loading...")}</p>}
              {error && <p className="text-center text-red-500">{error}</p>}
              {!loading && !error && (
                <p className="text-gray-600 leading-relaxed text-center text-base sm:text-lg">
                  {horoscopeText || t("noPrediction", "Horoscope data will be updated soon. Please check back later.")}
                </p>
              )}
            </div>
          </div>
